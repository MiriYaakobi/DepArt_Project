...
internal static class OrderManager
{
    ...
    // Helper for random data generation (Stage 7)
    private static readonly Random s_rand = new();

    // Mutex to prevent overlapping simulation executions (Stage 7)
    ...
    /// <summary>
    /// ASYNC version: creates a new order asynchronously.
    /// </summary>
    internal static async Task CreateOrderAsync(BO.Order order)
    {
        ...
        try
        {
            ...
            coordinates = await Tools.GetCoordinatesOfAddressAsync(order.Address ?? "");
        }
        catch
        {
    ...
    /// <summary>
    /// SIMULATOR LOGIC: Simulates the creation of a new order with random data asynchronously.
    /// </summary>
    internal static async Task SimulateNewOrderCreationAsync()
    {
        ...
        try
        {
            // Random probability: 10% chance to create an order per tick
            if (s_rand.NextDouble() > 0.1)
                return;

            // Generate random data
            int randomId = s_rand.Next(1000, 9999);
            string name = $"Simulated Customer {randomId}";
            string phone = $"05{s_rand.Next(10000000, 99999999)}";

            string[] streets = { "Arlozorov", "Ben Yehuda", "Dizengoff", "Allenby", "Rothschild", "Begin", "Kaplan", "Ibn Gabirol", "King George" };
            string address = $"{streets[s_rand.Next(streets.Length)]} {s_rand.Next(1, 100)}, Tel Aviv";
            ...
            BO.Order newOrder = new BO.Order()
            {
                ...
                CustomerPhone = phone,
                Address = address,
                TypeOfOrder = (BO.OrderType)s_rand.Next(0, 2), // Standard or Express randomly
                PackageDetails = $"Random Package #{randomId}",
        ...
        {
            // Ignore errors during simulation to keep the clock running
        }
        finally
        {
    ...
    /// <summary>
    /// SIMULATOR LOGIC: Periodically checks and updates orders that should be marked as delivered based on the new time.
    /// </summary>
    internal static async Task PeriodicOrderUpdatesAsync(DateTime newTime)
    {
        ...
        try
        {
            // Retrieve only InProgress orders
            ...
            // notify observers about potential updates
            Observers.NotifyListUpdated();
        }
        finally
        {
    ...
    internal static BO.Order ReadOrder(int orderId)
    {
        ...
        BO.OrderStatus statusOfOrder = CalculateOrderStatus(orderId); // Has lock inside
        BO.ScheduleStatus timeLinessStatus = CalculateScheduleStatus(orderId); // Has lock inside
        DateTime maxDeliveryTime = CalculateMaxDeliveryTime(orderId); // Has lock inside
        ...
        lock (AdminManager.BlMutex)
        {
            airDistance = Tools.GetAirDistance(
        ...
        IEnumerable<BO.DeliveryPerOrderInList> deliveryListCollection = MapDeliveryListForOrder(orderId); // Has lock inside
        ...
        {
            DO.Delivery? openDelivery;
            lock (AdminManager.BlMutex)
            {
                // find the only open delivery (DeliveryEndTime == null)
                openDelivery = s_dal.Delivery.ReadAll(d => d.OrderId == orderId && d.DeliveryEndTime == null).FirstOrDefault();
            }
    ...
    /// <summary>
    /// ASYNC version: gets a full order by ID.
    /// </summary>
    internal static async Task<BO.Order> ReadOrderAsync(int orderId)
    {
        // retrieve existing order
        ...
        if (statusOfOrder == BO.OrderStatus.InProgress)
        {
            ...
            // if found, calculate expected delivery time asynchronously
            if (openDelivery != null)
            {
                try
                {
                    // use await for async calculation
                    TimeSpan estimatedDuration = await CalculateEstimatedDeliveryDurationAsync(openDelivery, doOrder);
                    expectedDeliveryTime = openDelivery.DeliveryStartTime.Add(estimatedDuration);
                }
                catch { }
            }
    ...
    {
        // retrieve all orders and company coordinates
        ...
        lock (AdminManager.BlMutex)
        {
            ...
            (companyLat, companyLon) = GetCompanyCoordinates();
        }

        // map each DO.Order to BO.OrderInList
        return dalOrders.Select(doOrder =>
        {
            int orderId = doOrder.Id;

            // retrieve delivery data related to the order
            ...
            lock (AdminManager.BlMutex)
            {
                ...
                activeDelivery = s_dal.Delivery.ReadAll(d => d.OrderId == orderId && d.DeliveryEndTime == null).FirstOrDefault();

                // calculate total deliveries and handling duration
                totalDeliveries = s_dal.Delivery.ReadAll(d => d.OrderId == orderId).Count();
                // find last closed delivery
                lastClosedDelivery = s_dal.Delivery.ReadAll(d => d.OrderId == orderId)
    ...
    internal static void UpdateOrder(BO.Order order)
    {
        ...
        // check order status
        BO.OrderStatus currentStatus = CalculateOrderStatus(order.Id); // Has lock inside
    ...
    internal static TimeSpan CalculateEstimatedDeliveryDuration(DO.Delivery doDelivery, DO.Order doOrder)
    {
        ...
        // get company coordinates
        (double companyLat, double companyLon) = GetCompanyCoordinates(); // Has lock inside
    ...
    /// <summary>
    /// ASYNC version: calculates estimated delivery duration.
    /// </summary>
    internal static async Task<TimeSpan> CalculateEstimatedDeliveryDurationAsync(DO.Delivery doDelivery, DO.Order doOrder)
    {
        // find the courier details
        DO.Courier doCourier;
        lock (AdminManager.BlMutex)
        {
            ...
            {
                throw new BO.BlDoesNotExistException($"Courier ID {doDelivery.CourierId} not found.", ex);
            }
        }

        (double companyLat, double companyLon) = GetCompanyCoordinates();
        BO.DeliveryType deliveryType = (BO.DeliveryType)doCourier.TypeOfDelivery;

        // use await for async calculation
        var routingResult = await Tools.GetActualDistanceAndEstimatedTimeAsync(
            companyLat,
            companyLon,
            doOrder.Latitude,
            doOrder.Longitude,
        ...
        // Fallback (same calculation as sync)
        double airDistanceKm = Tools.GetAirDistance(companyLat, companyLon, doOrder.Latitude, doOrder.Longitude);
        double speedKmH = deliveryType switch
        {
            BO.DeliveryType.Car => 50,
    ...
    internal static DateTime CalculateMaxDeliveryTime(int orderId)
    {
        ...
        lock (AdminManager.BlMutex)
        {
            ...
            maxTimeSpan = s_dal.Config.MaxDeliveryRange;
        }
    ...
    {
        // retrieve closed deliveries
        lock (AdminManager.BlMutex)
        {
            // get all closed deliveries for the order
    ...
    internal static BO.ScheduleStatus CalculateScheduleStatus(int orderId)
    {
        ...
        lock (AdminManager.BlMutex)
        {
            ...
            finishedDelivery = s_dal.Delivery
    ...
    internal static IEnumerable<BO.DeliveryPerOrderInList> MapDeliveryListForOrder(int orderId)
    {
        ...
        lock (AdminManager.BlMutex)
        {
            allDeliveries = s_dal.Delivery.ReadAll(d => d.OrderId == orderId);
        }
    ...
    internal static void AssertReadAuthorization(int requestingUserId, int orderId)
    {
        ...
        lock (AdminManager.BlMutex)
        {
            openDelivery = s_dal.Delivery.ReadAll(d => d.OrderId == doOrder.Id && d.DeliveryEndTime == null).FirstOrDefault();
        }
    ...
    {
        IEnumerable<int> allOrderIds;
        // get all order IDs
        lock (AdminManager.BlMutex)
        {
            allOrderIds = s_dal.Order.ReadAll().Select(o => o.Id);
        }
        ...
        var groupedResults = allOrderIds.GroupBy(orderId =>
        {
            ...
            BO.OrderStatus status = CalculateOrderStatus(orderId); // Has lock inside
            BO.ScheduleStatus scheduleStatus = CalculateScheduleStatus(orderId); // Has lock inside
    ...
    internal static void CancelOrder(int orderId)
    {
        ...
        DO.Order doOrder = GetExistingOrder(orderId); // Has lock inside
        BO.OrderStatus currentStatus = CalculateOrderStatus(orderId); // Has lock inside
