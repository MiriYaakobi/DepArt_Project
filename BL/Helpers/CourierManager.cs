...
//    /// <summary>
//    /// SIMULATOR LOGIC: Called periodically by AdminManager to update courier status and complete deliveries.
//    /// </summary>
//    /// <param name="oldClock">Previous clock time</param>
//    /// <param name="newClock">Current clock time</param>
//    internal static void PeriodicCourierUpdates(DateTime oldClock, DateTime newClock)
//    {
//        var allCouriers = s_dal.Courier.ReadAll().ToList();

//        // שליפת הגדרות מהירות מהקונפיגורציה
//        double speedFoot = s_dal.Config.AverageByFootSpeedKmH;
//        double speedBike = s_dal.Config.AverageBicycleSpeedKmH;
//        double speedMoto = s_dal.Config.AverageMotorcycleSpeedKmH;
//        double speedCar = s_dal.Config.AverageVehicleSpeedKmH;

//        // הגנה מפני מהירות 0 (למקרה שהקונפיגורציה לא אותחלה טוב)
//        if (speedFoot <= 0) 
//            speedFoot = 5;
...
//        foreach (var doCourier in allCouriers)
//        {
//            // מדלגים על שליחים לא פעילים
//            if (!doCourier.IsActive) continue;

//            // 1. בדיקה: האם לשליח יש משלוח פתוח כרגע?
//            DO.Delivery? openDelivery = FindOpenDeliveryForCourier(doCourier.Id);

//            if (openDelivery != null)
//            {
//                // --- תרחיש א': השליח עובד - נבדוק אם סיים ---

//                // זיהוי המהירות לפי כלי הרכב
...
//                // חישוב זמן ההגעה המשוער
//                // הנוסחה: זמן = מרחק / מהירות
//                double distance = openDelivery.ActualDistance ?? 0;

//                // הגנה ממקרה קצה של מרחק 0
//                if (distance <= 0) distance = 0.1;

//                double hoursRequired = distance / speed;
//                TimeSpan timeRequired = TimeSpan.FromHours(hoursRequired);

//                // מתי הוא אמור להגיע? (זמן יציאה + זמן נדרש)
//                DateTime arrivalTime = openDelivery.DeliveryStartTime.Add(timeRequired);

//                // בדיקת הסימולטור: האם השעון הנוכחי עבר את זמן ההגעה?
//                if (newClock >= arrivalTime)
//                {
//                    try
//                    {
//                        // סיום המשלוח בהצלחה!
//                        DeliveryManager.CompleteDeliveryUpdate(doCourier.Id, openDelivery.Id, BO.OrderEndStatus.Delivered);
//                    }
//                    catch
//                    {
//                        // מתעלמים משגיאות בסימולטור כדי לא לתקוע את הריצה
//                    }
//                }
//            }
//            else
//            {
//                // --- תרחיש ב': השליח נח - נבדוק אי-פעילות (הלוגיקה המקורית) ---

//                DateTime? lastActivityTime = GetLastActivityTime(doCourier.Id);
//                TimeSpan inactivityTimeSpan = s_dal.Config.InactivityTimeRange;

//                // אם מוגדר זמן אי-פעילות, והשליח חרג ממנו
